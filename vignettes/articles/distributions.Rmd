...
Two versions of the inverse Pareto distribution are implemented in `ssdtools`, distinguished by their parameterisation (see below).
The North American version can be fitted by supplying the string `invpareto` to the `dists` argument in the `ssd_fit_dists` call, while the European version can be fitted by supplying the string `invpareto_eur`.
...
This North American version is the one fitted by `ssdtools` when the string `invpareto` is supplied to the `dists` argument.
...
Because it is *unbounded* above, the European version of the inverse Pareto distribution is a more realistic stand-alone SSD than the North American version.
It is fitted by `ssdtools` when the string `invpareto_eur` is supplied to the `dists` argument, and uses the same `shape` and `scale` parameterisation as `actuar::dinvpareto()`.
Both the North American `invpareto` and the European `invpareto_eur` are offered as user-selectable distributions that can be fitted on their own or as part of a model-averaged set, although neither is included in the default candidate set.
Note that the North American `invpareto` is bounded above, so model-averaged estimates that include it should be interpreted with caution in the upper tail.
