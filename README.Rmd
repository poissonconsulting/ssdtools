...
The ssdtools package uses Maximum Likelihood to fit distributions such as the gamma, log-logistic, log-normal and Weibull to censored and/or weighted data.
Multiple distributions can be averaged using Akaike Information Criteria.
Confidence intervals on derived estimates such as hazard concentrations and proportions of species protected are produced by parametric bootstrapping.
...
`ssdtools` accesses examples SSD data from the [ssddata](https://CRAN.R-project.org/package=ssddata) package for a range of chemicals. We will use the Boron data here as an example.
...
The model-averaged 5% hazard concentration is estimated, along with lower (lcl) and upper (ucl) confidence internvals via parametric bootstrapping using `ssd_hc`
.
```{r}
set.seed(99)
