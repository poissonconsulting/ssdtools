...
test_that("invpareto_burrlioz", {
  test_dist("invpareto_burrlioz", upadj = 1e-03)
  expect_equal(ssd_pinvpareto_burrlioz(0.5), 0.125)
  expect_equal(ssd_qinvpareto_burrlioz(0.125), 0.5)
  set.seed(42)
  expect_equal(ssd_rinvpareto_burrlioz(2), c(0.970755086941947, 0.978569136804486))
})

test_that("invpareto_burrlioz fits with anon_a", {
  fit <- ssd_fit_dists(ssddata::anon_a, dists = "invpareto_burrlioz")
  expect_s3_class(fit, "fitdists")
  tidy <- tidy(fit)
  expect_snapshot_data(tidy, "anon_a")
})

test_that("invpareto_burrlioz gives cis with ccme_boron", {
  fit <- ssd_fit_dists(ssddata::ccme_boron, dists = "invpareto_burrlioz")
  expect_s3_class(fit, "fitdists")
  set.seed(99)
...
test_that("invpareto_burrlioz ssd_hp gives cis with ccme_boron", {
  fit <- ssd_fit_dists(ssddata::ccme_boron, dists = "invpareto_burrlioz")
  expect_s3_class(fit, "fitdists")
  set.seed(99)
...
test_that("invpareto_burrlioz initial shape is MLEs", {
  set.seed(99)
  data <- data.frame(Conc = ssd_rinvpareto_burrlioz(6), weight = 1)
  data$left <- data$Conc
  data$right <- data$left
  initial <- ssdtools:::sinvpareto_burrlioz(data)
  expect_equal(lapply(initial, exp), list(log_scale = 1.03299515712949, log_shape = 4.14668077241))
  fit <- ssd_fit_dists(data, dists = "invpareto_burrlioz")
  expect_equal(
    estimates(fit),
    list(invpareto_burrlioz.weight = 1, invpareto_burrlioz.scale = 1.03299515712949, invpareto_burrlioz.shape = 4.14668077241)
  )
})

test_that("invpareto_burrlioz unbiased scale estimator small n", {
  set.seed(99)
  fun <- function(n) {
    exp(ssdtools:::sinvpareto_burrlioz(data.frame(right = ssd_rinvpareto_burrlioz(n)))$log_scale)
  }
  expect_equal(mean(vapply(rep(6, 1000), fun, 1)), 0.992849622620409)
})

test_that("invpareto_burrlioz biased shape estimator small n", {
  set.seed(99)
  fun <- function(n) {
    exp(ssdtools:::sinvpareto_burrlioz(data.frame(right = ssd_rinvpareto_burrlioz(n)))$log_shape)
  }
  expect_equal(mean(vapply(rep(6, 1000), fun, 1)), 3.8284232651135)
})

test_that("invpareto_burrlioz unbiased scale estimator large n", {
  set.seed(99)
  fun <- function(n) {
    exp(ssdtools:::sinvpareto_burrlioz(data.frame(right = ssd_rinvpareto_burrlioz(n)))$log_scale)
  }
...
test_that("invpareto_burrlioz unbiased shape estimator large n", {
  set.seed(99)
  fun <- function(n) {
    exp(ssdtools:::sinvpareto_burrlioz(data.frame(right = ssd_rinvpareto_burrlioz(n)))$log_shape)
  }
...
test_that("invpareto_burrlioz with extreme data", {
  ...
  fit98 <- ssd_fit_dists(data[1:98, , drop = FALSE], dists = "invpareto_burrlioz")
  expect_equal(
    estimates(fit98),
    list(invpareto_burrlioz.weight = 1, invpareto_burrlioz.scale = 2.61422908501617, invpareto_burrlioz.shape = 26.0909009531098)
  )
  
  fit99r <- ssd_fit_dists(data, dists = "invpareto_burrlioz", rescale = TRUE)
  expect_equal(
    estimates(fit99r),
    list(invpareto_burrlioz.weight = 1, invpareto_burrlioz.scale = 1.03020756694085, invpareto_burrlioz.shape = 26.0278618888664)
  )
})
